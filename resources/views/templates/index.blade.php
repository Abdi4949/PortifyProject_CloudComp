    ...
    <div class="min-h-screen bg-[#0f172a] text-slate-200 py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($templates as $template)
                    @php
                        ...
                        // Adaptasi dari code lama kamu: $isPro = isset($template['is_premium']) ...
                        // Tapi karena di controller sebelumnya kita pakai logic ID <= 5 free, sisanya pro:
                        $isPro = $template['id'] > 5; // Atau sesuaikan dengan logic database kamu: $template['is_premium']
                    ...
                    <div class="group relative bg-slate-800 rounded-3xl overflow-hidden border transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-purple-900/20 flex flex-col h-full
                        ...
                        <div class="relative h-56 bg-slate-700 overflow-hidden group-hover:opacity-100 transition-opacity">
                            <div class="absolute inset-0 bg-gradient-to-t from-slate-900 via-transparent to-transparent opacity-60 z-10"></div>
                            
                            @if(isset($template['image']) && $template['image'])
                                <img src="{{ asset('images/templates/' . $template['image']) }}" alt="{{ $template['name'] }}" class="w-full h-full object-cover object-top transition-transform duration-700 group-hover:scale-110">
                            @else
                                <div class="w-full h-full flex flex-col items-center justify-center text-slate-500 bg-slate-700/50">
                                    @if($template['id'] <= 5)
                                        <svg class="w-16 h-16 mb-2 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                        <span class="text-sm font-medium">Clean Layout</span>
                                    @else
                                        <svg class="w-16 h-16 mb-2 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                        <span class="text-sm font-medium">Visual Gallery</span>
                                    @endif
                                </div>
                            @endif

                            <div class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-20">
                                <a href="{{ route('templates.show', $template['id']) }}" class="bg-white text-slate-900 px-6 py-2 rounded-full text-sm font-bold shadow-xl hover:scale-105 transition-transform">
                                    👁️ Preview Design
                                </a>
                            </div>
                        </div>
            ...
            @if($templates->count() == 0)
                <div class="text-center py-20 bg-slate-800 rounded-3xl shadow-sm border border-slate-700">
